...
const CreateUser = async (req, res) => {
    ...
    try {
        const exits = await test.findOne({ Email: userpayload.Email})
        // console.log(exits);
        if (exits) {
...
GetbyId = async (req, res) => {
    try {
        const exits = await test.findOne({ id: req.params.id } )
        if (exits) {
            return res.status(200).send({
                status: 200,
                data: exits
...
UpdatebyId = async (req, res) => {
    ...
    try {
        ...
        const result = await test.findOneAndUpdate({id:req.params.id},{
            Name:userpayload.Name,
            Email:userpayload.Email,
            password:bcrypt.hashSync(userpayload.password, 10)}
        )
...
DeletebyId=async (req,res)=>{
    try {
        const exits = await test.findOneAndDelete( { id: req.params.id } )
        if (exits) {
            return res.status(200).send({
...
DeleteAllData=async (req,res)=>{
    try {
        const exits = await test.deleteMany()
